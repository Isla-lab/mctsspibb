...
from src import spibb_utils, spibb
...
class Garnets:
    ...
    def step(self, action, easter_egg):
        if self.transition_function[int(self.current_state), action, :].squeeze().sum() == 0:
    ...
    # Reward matrix
    def compute_reward(self, env_type=0):
        R = np.zeros((self.nb_states, self.nb_states))
        #hs = [0.2, 0.4, 0.6, 0.8]
        N=100
        hs = [x for x in np.linspace(1/N,1, N)]
        # hs = [0.25, 0.5, 0.75]
        #hs = [0.5]
        for s in range(self.nb_states):
            R[s, self.final_state] = 100
            if env_type == 2:
                for h in hs:
                    if s == int(self.nb_states*h):
                        R[s, s+1] = 1
        return R
    ...
    def _find_farther_state(self, gamma):
        ...
        cap = 10
        range_cap = np.amin([self.nb_states, cap])
        for final_state in range(1, range_cap): #TODO changes
            ...
            if perf_star < min_value and perf_star > gamma ** cap:
                min_value = perf_star
                argmin = final_state
