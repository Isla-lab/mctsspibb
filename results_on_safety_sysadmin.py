...
from src.util import load_env_and_policies_sysadmin, save_object
import warnings
...
def policy_comparison():
    ...
    nb_trajectories_list = [2, 10, 100, 1000, 10000]
    n_wedge = 50
    n_iterations = 20
    discount_factor = 0.85
    real_T, R, optimal, baseline, garnet, final_states, path = load_env_and_policies_sysadmin(n_machines,
                                                                                              discount_factor)
    ...
    save_object(perf_baseline, 'out/SysAdmin/results/v0_baseline_%s.pkl' % n_machines)
    save_object(perf_optimal, 'out/SysAdmin/results/v0_optimal_%s.pkl' % n_machines)
    for nb_trajectories in nb_trajectories_list:
        ...
        for it in range(n_iterations):
            ...
            save_object(model.transitions, 'out/SysAdmin/mle/sysadmin_%s_trj_%s_mle_T_it_%s.pkl'
                        % (n_machines, nb_trajectories, it))
            mask, q = spibb.compute_mask_N_wedge(n_states, n_actions, n_wedge, batch_traj)
            save_object(mask, 'out/SysAdmin/mask/sysadmin_%s_trj_%s_mask_it_%s.pkl'
                        % (n_machines, nb_trajectories, it))
            save_object(q, 'out/SysAdmin/q/sysadmin_%s_trj_%s_q_it_%s.pkl'
                        % (n_machines, nb_trajectories, it))
            save_object(model.transitions, 'out/SysAdmin/batch/sysadmin_%s_trj_%s_batch_it_%s.pkl'
                        % (n_machines, nb_trajectories, it))
            # SPIBB
            seed = 1
            np.random.seed(seed)
            ...
            for n_sim in n_sims:
                ...
                perf_mcts[n_sim].append(values_mcts)
        ...
        save_object(perf_spibb, 'out/SysAdmin/results/v0_spibb_trj_%s_it_%s.pkl' % (nb_trajectories, it))
        save_object(perf_mcts, 'out/SysAdmin/results/v0_mcts_trj_%s_it_%s.pkl' % (nb_trajectories, it))
