...
class StockTradingEnv(discrete.DiscreteEnv):
    ...
    from [Strehl et al., 2007] Efficient Structure Learning in Factored-state MDPs
    """
    ...
    def __init__(self, number_of_sectors, number_of_stocks_per_sector):
        ...
        isd = np.zeros(nS)
        isd[0] = 1  # starts owing nothing and no stock rising
    ...
    def get_true_parents(self):
        ...
        for sector in range(self.number_of_sectors):
            ...
            for a in range(self.nA):
                if a == sector or a == sector + self.number_of_sectors:
                    parents[(own_sector, a)] = tuple()
                else:
                    parents[(own_sector, a)] = tuple([own_sector])
                for stock in self.stocks_from_sector(sector):
                    parents[(stock, a)] = tuple(self.stocks_from_sector(sector))
        return parents
    ...
    def render(self, mode='human'):
        ...
        out = "".join([ul(x) for x in state_factors])
        outfile.write(out+"\n")
        if self.lastaction is not None:
            action_line = "".join("^" if i == self.lastaction * (self.stocks_per_sector + 1)
                                  else " " for i in range(self.number_of_sectors * self.stocks_per_sector))
        else:
            action_line = ""
